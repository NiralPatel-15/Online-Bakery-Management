...
if (strlen($_SESSION['fosaid']) == 0) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $price = $_POST['price'];
        $weight = $_POST['weight']; // ✅ NEW: weight added

        $query = mysqli_query($con, "UPDATE tblproduct 
            SET CategoryName='$fcat',
                ItemName='$itemname',
                ItemPrice='$price',
                ItemDes='$description',
                ItemQty='$quantity',
                Weight='$weight'  -- ✅ NEW: Weight updated
            WHERE ID='$cid'");

        if ($query) {
            header("Location: manage-item.php?status=updated");
            exit();
        } else {
                                        ...
                                        <form id="submit" action="#" class="wizard-big" method="post" name="submit">
                                            <fieldset>
                                                ...
                                                <div class="form-group row"><label class="col-sm-2 col-form-label">Description:</label>
                                                    <div class="col-sm-10">
                                                        <textarea class="form-control" name="description" rows="8" cols="12" required><?php echo $row['ItemDes']; ?></textarea>
                                                    </div>
                                                </div>

                                                <div class="form-group row"><label class="col-sm-2 col-form-label">Image</label>
                                                    <div class="col-sm-10">
                                                        <img src="itemimages/<?php echo $row['Image']; ?>" width="200" height="150">
                                                ...
                                                <div class="form-group row"><label class="col-sm-2 col-form-label">Category:</label>
                                                    <div class="col-sm-10">
                                                        <select class="form-control white_bg" name="foodcategory">
                                                            ...
                                                            ?>
                                                                <option value="<?php echo $row2['CategoryName']; ?>"><?php echo $row2['CategoryName']; ?></option>
                                                            <?php } ?>
                                                        </select>
                                                    </div>
                                                </div>
