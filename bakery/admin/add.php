...
if (strlen($_SESSION['fosaid']) == 0) {
    ...
    if (isset($_POST['submit'])) {
        ...
        // Use pathinfo for extension check
        $extension = strtolower(pathinfo($itempic, PATHINFO_EXTENSION));
        $allowed_extensions = array("jpg", "jpeg", "png", "gif");

        if (!in_array($extension, $allowed_extensions)) {
            header("Location: add.php?status=invalidformat");
            exit();
        } elseif ($price <= 0) {
            header("Location: add.php?status=invalidprice");
            exit();
        } else {
            $newfilename = md5($itempic) . '.' . $extension;
            move_uploaded_file($_FILES["itemimages"]["tmp_name"], "itemimages/" . $newfilename);

            $query = mysqli_query($con, "INSERT INTO tblproduct(CategoryName, ItemName, ItemPrice, ItemDes, ItemQty, Weight, Image) 
                VALUES('$fcat', '$itemname', '$price', '$description', '$quantity', '$weight', '$newfilename')");

            if ($query) {
                header("Location: add.php?status=success");
                exit();
            } else {
                header("Location: add.php?status=error");
                exit();
            }
        }
    }
?>
    ...
    <head>
        <title> Bakery House|| Add Item </title>
            ...
            $(document).ready(function() {
                ...
                $("#form").steps({
                    ...
                    onFinishing: function(event, currentIndex) {
                        ...
                        // Disable validation on fields that are disabled. At this point it's recommended to do an overall check (mean ignoring only disabled fields)
