...
if (strlen($_SESSION['fosuid']) == 0) {
	...
	if (isset($_POST['placeorder'])) {
		...
		$cart_check = mysqli_query($con, "SELECT ID FROM tblorders WHERE UserId='$userid' AND IsOrderPlaced IS NULL");
		...
		$fnaobno = mysqli_real_escape_string($con, $_POST['flatbldgnumber']);
		$street = mysqli_real_escape_string($con, $_POST['streename']);
		$area = mysqli_real_escape_string($con, $_POST['area']);
		$lndmark = mysqli_real_escape_string($con, $_POST['landmark']);
		$city = mysqli_real_escape_string($con, $_POST['city']);
		$cod = mysqli_real_escape_string($con, $_POST['cod']);

		$orderno = mt_rand(100000000, 999999999);

		// Multiple queries: update order table and insert into address
		$update_query = "UPDATE tblorders SET OrderNumber='$orderno', IsOrderPlaced='1', CashonDelivery='$cod' WHERE UserId='$userid' AND IsOrderPlaced IS NULL;";
		$insert_query = "INSERT INTO tblorderaddresses(UserId, Ordernumber, Flatnobuldngno, StreetName, Area, Landmark, City)
                         VALUES('$userid','$orderno','$fnaobno','$street','$area','$lndmark','$city');";

		if (mysqli_multi_query($con, $update_query . $insert_query)) {
			echo "<script>alert('Your order has been placed successfully! Order number is $orderno');</script>";
						...
						<div class="payment_list">
							<div class="price_single_cost">
								...
								while ($row = mysqli_fetch_array($query)) {
									...
									$grandtotal += $total;
								?>
									<h5><?php echo htmlentities($row['ItemName']); ?> (x<?php echo $row['ItemQty']; ?>) <span>₹<?php echo $total; ?></span></h5>
								<?php } ?>
							...
							<div id="accordion" class="accordion_area">
								<div class="card">
									<div class="card-header" id="headingOne">
										<h5 class="mb-0">
											<input type="checkbox" name="cod" value="Cash on Delivery" required>
